<script>
    ...
    afterUpdate(async ()=>{
                ...
                if (refresh){
                    refresh=false;
                dataGraph0 = await data0("est/php/GetPromediosAsignatura.php");
                }
    ...
    const graphics = (type, canvas, dataGraph) => {
        ...
        let options = {
            type,
            data: {
                labels: dataGraph.map((data) => data.valoracion),
                datasets: [
                    {
    ...
    $: if (myCanvas0 && dataGraph0) {
        ...
        myChart0 = graphics(
            "bar",
            myCanvas0,
            dataGraph0.filter((data) => data.valoracion).sort((a,b)=>a.valoracion-b.valoracion)
        );
    }
